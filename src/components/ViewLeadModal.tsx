...
interface ViewLeadModalProps {
  lead: Lead;
  onClose: () => void;
  onAddActivity?: (leadId: number, activity: any) => void;
}

export default function ViewLeadModal({ lead, onClose, onAddActivity }: ViewLeadModalProps) {
      ...
      <div className="bg-white rounded-xl shadow-lg w-full max-w-2xl">
        ...
        <div className="flex justify-between items-center p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Viewing information for {lead.name}</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div>
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${statusStyles[lead.status] || 'bg-gray-100 text-gray-800'}`}>
                  {lead.status}
                </span>
              </div>
            </div>
          ...
          <div className="mt-6">
            ...
            <div className="text-base text-gray-900 bg-gray-50 p-4 rounded-lg min-h-[100px]">
              {lead.notes ? (
                <div className="whitespace-pre-wrap">{lead.notes}</div>
              ) : (
                <div className="text-gray-400 italic">No notes available for this lead</div>
              )}
            </div>
          </div>
                ...
                <div className="space-y-3">
                  {[...lead.activities].reverse().map(activity => (
                    <div key={activity.id} className="border-l-4 border-blue-500 pl-3 py-1">
                      ...
                      <p className="text-sm text-gray-600 mt-1">{activity.snippet}</p>
                    </div>
                  ))}
                </div>
              </div>
